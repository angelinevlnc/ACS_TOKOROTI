...
namespace MasterTokoRoti.Models
{
    public class JenisRoti
    {
        ...
        public JenisRoti() { }
        public JenisRoti(string id, string nama_jenis, string keterangan)
        {
            this.ID = id;
        ...
        public static DataTable fetch()
        {
            ...
            SqlCommand cmd = new SqlCommand("select id, nama_jenis, keterangan from jenis_roti", DB.conn);
        ...
        public void insert()
        {
            SqlCommand cmd1;
            DB.openConnection();
            cmd1 = new SqlCommand("SELECT count(*)+1 FROM jenis_roti;", DB.conn);
            string cek = cmd1.ExecuteScalar().ToString();
            int hasilcek = Int32.Parse(cek);
            DB.closeConnection();

            SqlCommand cmd = new SqlCommand("INSERT INTO jenis_roti VALUES(@ID,@NAMA_JENIS,@status,@keterangan)", DB.conn);
        ...
        public void delete()
        {
            SqlCommand cmd = new SqlCommand("UPDATE jenis_roti SET STATUS=0 WHERE id=@id", DB.conn);
        ...
        public void update()
        {
            ...
            SqlCommand cmd = new SqlCommand("UPDATE jenis_roti SET nama_jenis = @nama_jenis, keterangan = @keterangan WHERE id=@id", DB.conn);
            cmd.Parameters.AddWithValue("@id", this.ID);
            cmd.Parameters.AddWithValue("@nama_jenis", this.NAMA_JENIS);
            cmd.Parameters.AddWithValue("@keterangan", this.KETERANGAN);
